...
runKit(async (kit) => {
  ...
  if (process.platform === 'win32' && parseInt(process.versions.node) <= 14) { // HACK: bypass npm@6+win32 `npm ERR! Error: EPERM: operation not permitted, rename` error
    ...
    kit.RUN('node lib/npm.js install --no-fund --no-audit --global @min-pack/npm@0.1', { cwd: kit.fromOsTemp('patch-npm-install-global/', 'package/') })
  } else kit.RUN_SUDO_NPM('install --no-fund --no-audit --global @min-pack/npm@0.1')
}, { title: 'ci-patch' })
